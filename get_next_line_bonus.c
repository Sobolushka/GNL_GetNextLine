...
static int			read_next_block(int fd, t_gnl *gnl)
{
	int				status;
...
static int			get_next_char(int fd, char *char_to, t_gnl *gnl)
{
	int				status;

	if (gnl->first && gnl->status_read < 1 && gnl->fd == fd)
	{
		*char_to = '\0';
		return (gnl->status_read);
	}
	if (!gnl->first || gnl->position == BUFFER_SIZE || gnl->fd != fd)
